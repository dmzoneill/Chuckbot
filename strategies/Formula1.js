...
class Formula1 extends MessageStrategy {
  ...
  static async apiRequest(url) {
    try {
      ...
      let page = request('GET', "https://" + Formula1.endpoint + "/" + url, {
        headers: {
  ...
  static async convert_image(save_path) {    
    let cmd = "convert '" + save_path + "' -background white -alpha remove -alpha off '" + save_path + "'.jpg";
  ...
  static async Races(message, season = new Date().getFullYear(), next = 0, last = 0, type = "race") {
    ...
    try {
      ...
      let filter_next = next > 0 ? "&next=" + next.toString() : "";
      let filter_last = last > 0 ? "&last=" + last.toString() : "";
      let filters = filter_next + filter_last;
      let result = await Formula1.apiRequest('races?season=' + season + filters + "&type=" + type);
      ...
      for (let y = 0; y < Formula1.races.length; y++) {
        let msg = "";
        let race = Formula1.races[y];
        let competition = race['competition'];
  ...
  static async RankingsTeam(message) {
    ...
    try {
      MessageStrategy.typing(message);
      let result = await Formula1.apiRequest('rankings/teams?season=' + new Date().getFullYear());
      if (result.errors.length > 0) {
        console.log(result.errors)
        return;
      }
      ...
      for (let y = 0; y < Formula1.ranking_team.length; y++) {
        let pos = Formula1.ranking_team[y];
        console.log(pos);
        let team = pos['team'];
  ...
  static async RankingsDriver(message) {
    ...
    try {
      MessageStrategy.typing(message);
      let result = await Formula1.apiRequest('rankings/drivers?season=' + new Date().getFullYear());
      ...
      for (let y = 0; y < Formula1.ranking_driver.length; y++) {
        let pos = Formula1.ranking_driver[y];
        console.log(pos);
        let driver = pos['driver'];
        let team = pos['team'];
  ...
  static async Driver(message) {
    ...
    try {
      MessageStrategy.typing(message);
      let search = message.body.substring(10);
      let result = await Formula1.apiRequest('drivers' + "?search=" + search);
