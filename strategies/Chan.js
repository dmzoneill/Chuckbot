...
class Chan extends MessageStrategy {
  ...
  async Pop(message) {
    try {
      let chatkeys = Object.keys(MessageStrategy.state['Chan']['Notified'])
      for (let y = 0; y < chatkeys.length; y++) {
        let subkeys = Object.keys(MessageStrategy.state['Chan']['Notified'][chatkeys[y]])
  ...
  async List(message) {
    try {
      let msg = ""
      let subkeys = Object.keys(MessageStrategy.state['Chan']['Subscribed'][message.from])
  ...
  async UnSubscribe(message) {
    try {
      ...
      if (Number.isNaN(isno) == false) {
        let subkeys = MessageStrategy.state['Chan']['Subscribed'][message.from]
        for (let h = 0; h < subkeys.length; h++) {
          if (h == isno) {
      ...
      var index = MessageStrategy.state['Chan']['Subscribed'][message.from].indexOf(to_be_removed);
      if (index !== -1) {
  ...
  async Subscribe(message) {
    try {
      ...
      let sitemaps = await Chan.self.GetSiteMaps(message, parts[2])
      ...
      let urls = await Chan.self.GetLocsFromXML(message, sitemaps[1][index]);
  ...
  async processChats(message) {
    let chats = Object.keys(MessageStrategy.state['Chan']['Subscribed']);

    for (let c = 0; c < chats.length; c++) {
      let subscriptions = MessageStrategy.state['Chan']['Subscribed'][chats[c]];
  ...
  async handleSubscription(message, chat, subscription) {
    let urls = await Chan.self.GetLocsFromXML(message, subscription);
  ...
  async processQueue() {
    if (Object.keys(MessageStrategy.state['Chan']['Queue']).length > 0) {
      let msgs = Object.keys(MessageStrategy.state['Chan']['Queue'])
