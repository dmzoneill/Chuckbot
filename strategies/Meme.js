...
class Meme extends MessageStrategy {
  ...
  async setup_ticker (chatid) {
    ...
    const min = MessageStrategy.state.Meme.chats[chatid].min
    const max = MessageStrategy.state.Meme.chats[chatid].max
    const rand = Math.floor(Math.random() * (max - min + 1)) + min
    const timer = setInterval(Meme.self.scheduled_meme, rand, chatid)
