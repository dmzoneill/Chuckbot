...
class Stock extends MessageStrategy {
  ...
  async LoadSymbols() {
    let rawdata = fs.readFileSync('strategies/symbols/symbols.txt');
  ...
  async SearchSymbol(message, symbol) {
    ...
    if (symbol.indexOf(" ") > -1) {
      ...
      let periods = ['1d', '7d', '1m', '6m', 'ytd', '1y', '5y', 'at']
      let extras = ['extra']
      let stats = ['stats']
  ...
  async GetStock(message) {
    try {
      let req = message.body.trim().substring(6).trim()
      ...
      let symbols = await Stock.self.SearchSymbol(message, req)
      ...
      if (symbols.length == 1) {
        ...
        let periods = ['1d', '7d', '1m', '6m', 'ytd', '1y', '5y', 'at']
        let types = ['fund', 'stock', 'forex', 'spot', 'swap', 'futures', 'index', 'cfd', 'fundamental', 'economic', 'dr', 'bond']
        let extras = ['extra']
        let stats = ['stats']
  ...
  async SearchStock(message) {
    try {
      let req = message.body.trim().substring("stock search".length).trim()
      let symbols = []
